...
sub displayAccessions {
	...
	if ( $gdb::webUtil::frmData{ginfo} and $gdb::webUtil::frmData{ginfo} =~ /^accessions/ ) {
		#ajax call
		...
		if ( $parms->{accessions} ) {
			print qq{<div class="mn2" style="border-top:1px solid #C3CCD3;"><span onclick="da('accessions');" onmouseover="mm(this,'accessions');" onmouseout="return nd();"><img id="accessionssign" src="$gdb::util::webloc/web/minus.gif" alt=""> Accessions</span></div>};
			print qq{<div class="showrec" id="accessions">};
			accessions();
			print qq{</div>\n};
		} else {
			print qq{<div class="mn2" style="border-top:1px solid #C3CCD3;"><span onclick="da('accessions');" onmouseover="mm(this,'accessions');" onmouseout="return nd();"><img id="accessionssign" src="$gdb::util::webloc/web/plus.gif" alt=""> Accessions</span></div>};
...
sub accessions {
	...
	my $dbLabOrgRecRef = gdb::oracle::dbLabOrganization();
	my %dbLabOrgRec    = %$dbLabOrgRecRef;

	my $dbExpmRefRecRef = gdb::oracle::dbExpmReference();
	my %dbExpmRefRec    = %$dbExpmRefRecRef;

	my $dbExpmDesignRecRef = gdb::oracle::dbExpmDesign();
	my %dbExpmDesignRec    = %$dbExpmDesignRecRef;

	my $dbPlatformDesignRecRef = gdb::oracle::dbPlatformDesign();
	my %dbPlatformDesignRec    = %$dbPlatformDesignRecRef;


	if ($gdb::webUtil::frmData{accsort}) {
		($col, $dir) = split( /:/, $gdb::webUtil::frmData{accsort} );
	}else{
	...
	for my $i ( sort { $a <=> $b } keys %dbAccession ) {
		...
		my $expGenomeRef = gdb::oracle::dbexpcntbyAcc($id);
		my @expGenome = @$expGenomeRef;
		my $expCnt = 0;
		
		foreach my $genome (@expGenome) {
			#next if ($genome !~ /$gdb::util::gnom{$parms->{genome}}{acc}/i);
			$expCnt++;
		}
		...
		if ( $expCnt > 0 ) {
			$tmpline .= qq{<img id="esign$id" src="$gdb::util::webloc/web/plus.gif" onclick="ckaccexp($id);" alt="" onmouseover="this.style.cursor='pointer';return overlib('click to display experiments');" onmouseout="return nd();"> };
		} else {
			$tmpline .= "&nbsp;&nbsp;&nbsp;&nbsp;";
		}
	...
	my $accHead = ($col =~ /accession/) ? qq{ACCESSION<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "ACCESSION";
	my $expsHead = ($col =~ /exps/) ? qq{EXPS<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "EXPS";
	my $institutionHead = ($col =~ /institution/) ? qq{INSTITUTION_[PI]<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "INSTITUTION_[PI]";
	my $referenceHead = ($col =~ /reference/) ? qq{REFERENCE<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "REFERENCE";
	my $titleHead = ($col =~ /title/) ? qq{ACCESSION TITLE<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "ACCESSION TITLE";
	my $experimentHead = ($col =~ /experiment/) ? qq{EXPERIMENT DESIGN<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "EXPERIMENT DESIGN";
	my $arrayHead = ($col =~ /array/) ? qq{ARRAY DESIGN<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "ARRAY DESIGN";
	my $genomeHead = ($col =~ /genome/) ? qq{SAMPLE SOURCE<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "SAMPLE SOURCE";
	
	#czhang
	my $organismHead = ($col =~ /organism/) ? qq{ORGANISM<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">} : "ORGANISM";	    
...
for my $i ( sort myColsort keys %line ) {
		...
		if ($sexp{$line{$i}{id}}) {	
			$gdb::webUtil::frmData{id} = $line{$i}{id};
			$gdb::webUtil::frmData{ckaccn} = 'true';
...
sub accExperiments {
	...
	my $ckaccn = ( $gdb::webUtil::frmData{ckaccn} =~ /true/ ) ? 'checked' : '';    #see if accession was checked
	
	print qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
	  qq{<td valign="top"><a class="exmp" onclick="ckaccexp($gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	...
	for my $id (@expm_order) {
		if ( $gdb::webUtil::frmData{id} == $dbaccExpm{$id}{expid} ) {
			...
			my $testgenome = ($dbaccExpm{$id}{testgenome}) ? $gdb::util::gnom{$gdb::util::gnomacc{$dbaccExpm{$id}{testgenome}}}{sname} : '';
			my $cntlgenome = ($dbaccExpm{$id}{cntlgenome}) ? $gdb::util::gnom{$gdb::util::gnomacc{$dbaccExpm{$id}{cntlgenome}}}{sname} : '';
			
			print qq{<tr bgcolor="#ebf0f2">\n},
			  qq{<td class="tdc"><input id="ckexpm$id" class="small" type="checkbox" name="ckexpm" value="$gdb::webUtil::frmData{id}:$id" onclick="ckexpmt(this,'ckaccn','ckallexp','$dbaccExpm{$id}{expid}');ckqry('$parms->{currquery}');" $ckaccn></td>\n},
...
sub accinfo {
	...
	my $row = gdb::oracle::dbgetAccName( $gdb::webUtil::frmData{id} );

	print
	  qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
	  qq{<td valign="top"><a class="exmp" onclick="accinfo('accinfo',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	  ...
	  qq{<tr>\n},
	  qq{<td class="tdc">&nbsp; <a onclick="nd();accinfo('providers',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Provider/Organization information');" onmouseout="return nd();">Providers</a> &nbsp; | &nbsp;</td>\n},
	  qq{<td class="tdc"><a onclick="nd();accinfo('summary',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Summary information');" onmouseout="return nd();">Summary</a> &nbsp; | &nbsp;</td>\n},
	  qq{<td class="tdc"><a onclick="nd();accinfo('expdesign',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Experiment Design information');" onmouseout="return nd();">Experiment Design</a> &nbsp; | &nbsp;</td>\n},
	  qq{<td class="tdc"><a onclick="nd();accinfo('arraydesign',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Array Design information');" onmouseout="return nd();">Array Design</a> &nbsp; | &nbsp;</td>\n},
	  qq{<td class="tdc"><a onclick="nd();accinfo('sampinfo',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Samples information');" onmouseout="return nd();">Samples</a> &nbsp; | &nbsp;</td>\n},
	  qq{<td class="tdc"><a onclick="nd();accinfo('expinfo',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Experiments information');" onmouseout="return nd();">GenExpDB Experiments</a> &nbsp; | &nbsp;</td>\n};

	print
	  qq{<td class="tdc"><a onclick="nd();accinfo('curated',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('GenExpDB Curated Info');" onmouseout="return nd();">Curated</a> &nbsp; | &nbsp;</td>\n}
	  if ($gdb::webUtil::useracclevel and $gdb::webUtil::useracclevel > 2 );

	print
	  qq{<td class="tdc"><a onclick="nd();accinfo('expdata',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Experiments Data');" onmouseout="return nd();">Data</a> &nbsp; | &nbsp;</td>\n},
	  qq{<td class="tdc"><a onclick="nd();gm('downloadAccessions',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Download Experiment Data');" onmouseout="return nd();">Download</a> &nbsp;&nbsp;</td>\n},
	  qq{</tr>\n},
	  qq{</table>\n},
	  qq{</td>\n},
	  qq{</tr>\n},
...
sub providers {
	...
	my $dbexpProvidersRef = gdb::oracle::dbexpProviders( $gdb::webUtil::frmData{id} );
	my %dbexpProviders    = %$dbexpProvidersRef;

	print qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
qq{<td valign="top"><a class="exmp" onclick="accinfo('providers',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	  qq{<table class="tblb" align="center">\n},
...
sub summary {
	...
	my $dbexpSummaryRef = gdb::oracle::dbexpSummary( $gdb::webUtil::frmData{id} );
	my %dbexpSummary    = %$dbexpSummaryRef;

	print qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
qq{<td valign="top"><a class="exmp" onclick="accinfo('summary',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	  qq{<table class="tblb" align="center">\n},
	  qq{<tr>\n},
...
sub expdesign {
	...
	my $dbexpDesignRef = gdb::oracle::dbexpDesign( $gdb::webUtil::frmData{id} );
	my %dbexpDesign    = %$dbexpDesignRef;

	print qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
qq{<td valign="top"><a class="exmp" onclick="accinfo('expdesign',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	  qq{<table class="tblb" align="center">\n},
...
sub arraydesign {
	...
	my $dbarrayDesignRef = gdb::oracle::dbarrayDesign( $gdb::webUtil::frmData{id} );
	...
	print qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
qq{<td valign="top"><a class="exmp" onclick="accinfo('arraydesign',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	  qq{<table class="tblb" align="center">\n},
...
sub sampinfo {
	...
	my $dbsampInfoRef = gdb::oracle::dbsampInfo( $gdb::webUtil::frmData{id} );
	...
	print qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
qq{<td valign="top"><a class="exmp" onclick="accinfo('sampinfo',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	  qq{<table class="tblb" align="center">\n},
	...
	for my $col ( sort { $a <=> $b } keys %colsamp ) {
		...
		for my $row ( sort { $a <=> $b } keys %{ $colsamp{$col} } ) {
			print
qq{<td class="tdc" bgcolor="#ebf0f2"><a onclick="nd();sampdetail('sampdetail$i$gdb::webUtil::frmData{id}','$dbsampInfo{$colsamp{$col}{$row}}{bioassays_id}');" onmouseover="this.style.cursor='pointer';return overlib('Sample details');" onmouseout="return nd();">$dbsampInfo{$colsamp{$col}{$row}}{samid}</a></td>\n},
...
qq{<td class="tdc" bgcolor="#ebf0f2"><a onclick="window.open('$ENV{REQUEST_URI}?ajax=accinfo&accinfo=viewRawData&view=$dbsampInfo{$colsamp{$col}{$row}}{fname}&accession=$dbsampInfo{$colsamp{$col}{$row}}{accession}&sname=$dbsampInfo{$colsamp{$col}{$row}}{samid}&id=$dbsampInfo{$colsamp{$col}{$row}}{bioassays_id}', '_blank')" onmouseover="this.style.cursor='pointer';return overlib('View raw data (new window)');" onmouseout="return nd();">view</a></td>\n},
		...
		print qq{<tr><td colspan="$spancol"><div class="hidden" id="sampdetail$i$gdb::webUtil::frmData{id}"></div></td></tr>\n};
		$i++;
	}
...
sub sampdetail {
	...
	my ( $sampIDname, $dbsampDetailRef ) = gdb::oracle::dbsampDetail( $gdb::webUtil::frmData{sampID} );
	my %dbsampDetail         = %$dbsampDetailRef;
	my $dbsampDescriptionRef = gdb::oracle::dbsampDescription( $gdb::webUtil::frmData{sampID} );
	my %dbsampDescription    = %$dbsampDescriptionRef;

	print qq{<table cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
	  qq{<td valign="top"><a class="exmp" onclick="sh('$gdb::webUtil::frmData{id}');" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
...
sub viewRawData {
	...
	my $opened = open( FILE, "$gdb::util::datapath/$gdb::webUtil::frmData{accession}/$gdb::webUtil::frmData{view}" );

	print qq{<html><head><title>$gdb::webUtil::frmData{sname} Raw Data</title>\n},
	...
	if ($opened) {
		...
		my $rec     = @data;
		my $dispnum = 30;
		my $full    = ( $gdb::webUtil::frmData{full} ) ? 1 : 0;

		my ($labelRef) = gdb::oracle::dbsampFilehead( $gdb::webUtil::frmData{id} );
		my %label = %$labelRef;

		print qq{<pre>\n}, qq{Accession: <b>$gdb::webUtil::frmData{accession}</b>\tSample: <b>$gdb::webUtil::frmData{sname}</b>\n}, qq{Rows: <b>$rec</b>};

		if ($full) {
			print qq{\n};
		} else {
			print qq{\t**Displaying $dispnum rows\t},
qq{<input class="ebtn" type="button" value="View full table" onclick="window.location.href='$gdb::util::url/?ajax=accinfo&accinfo=viewRawData&view=$gdb::webUtil::frmData{view}&accession=$gdb::webUtil::frmData{accession}&sname=$gdb::webUtil::frmData{sname}&id=$gdb::webUtil::frmData{id}&full=1'">\n};
		}
...
sub expinfo {
	...
	my ( $dbExpmRef, $expmorderRef ) = gdb::oracle::dbgetExpmInfo();    #all experiments, needed for ouid
	my %dbExpm = %$dbExpmRef;

	my $expid        = $gdb::webUtil::frmData{id};
	
	my $dbexpInfoRef = gdb::oracle::dbexpInfo($expid);
	my %dbexpInfo    = %$dbexpInfoRef;
	my $expCnt       = scalar keys %dbexpInfo;

	print qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
qq{<td valign="top"><a class="exmp" onclick="accinfo('expinfo',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	...
	print qq{<tr><td class="tdc" colspan="3"><input class="ebtn" type="button" value="Update" onclick="updexperiment($expid,this.form)"></td><td class="small">$status</td></tr>\n}
	  if ( $expCnt > 0 and $gdb::webUtil::useracclevel > 2 );
	...
	for my $id ( sort { $a <=> $b } keys %dbexpInfo ) {
		...
		my $dbcolumnNameRef = gdb::oracle::dbcolumnName( $dbexpInfo{$id}{samples} );    #get all columns for this sample
		...
		my $testgenome    = ($dbexpInfo{$id}{testgenome}) ? $gdb::util::gnom{$gdb::util::gnomacc{$dbexpInfo{$id}{testgenome}}}{sname} : '';
		...
		my $cntlgenome    = ($dbexpInfo{$id}{cntlgenome}) ? $gdb::util::gnom{$gdb::util::gnomacc{$dbexpInfo{$id}{cntlgenome}}}{sname} : '';
...
sub updexperiment {
	...
	my $dbexpInfoRef = gdb::oracle::dbexpInfo( $gdb::webUtil::frmData{id} );
	my %dbexpInfo    = %$dbexpInfoRef;

	$gdb::webUtil::frmData{en} =~ s/\x2B/\+/g;    #these were changed to get thru html post
	$gdb::webUtil::frmData{en} =~ s/\x3D/\=/g;

	my @delexp    = split( /\|\~\|/, $gdb::webUtil::frmData{ed} );
	my @exporder  = split( /\|\~\|/, $gdb::webUtil::frmData{eo} );
	my @expname   = split( /\|\~\|/, $gdb::webUtil::frmData{en} );
	my @exptimept = split( /\|\~\|/, $gdb::webUtil::frmData{et} );
	...
	for my $id ( sort { $a <=> $b } keys %dbexpInfo ) {
		if ( $delexp[$id] =~ /true/ ) {
			...
			push @allchg, "update pexp set $change moddate=sysdate,moduser='$gdb::webUtil::username' where id=$dbexpInfo{$id}{id}" if $change;
		}
	}

	my $status;
	if (@allchg) {
		my $rc = gdb::oracle::dbupdateExpInfo( \@allchg );
...
sub curated {
	...
	my $id               = $gdb::webUtil::frmData{id};
	my $dbcuratedInfoRef = gdb::oracle::dbcuratedInfo($id);
	my %dbcuratedInfo    = %$dbcuratedInfoRef;

	my $dbexpDesignRef    = gdb::oracle::dbexpDesign($id);                       #exp design
	my %dbexpDesign       = %$dbexpDesignRef;
	my $dbexpProvidersRef = gdb::oracle::dbexpProviders($id);                    #org and provider
	my %dbexpProviders    = %$dbexpProvidersRef;
	my $dbExpmRefRecRef   = gdb::oracle::dbExpmReference();                      #pubmed
	my %dbExpmRefRec      = %$dbExpmRefRecRef;
...
sub updcurated {
	...
	my $dbcuratedInfoRef = gdb::oracle::dbcuratedInfo( $gdb::webUtil::frmData{id} );
	my %dbcuratedInfo    = %$dbcuratedInfoRef;
	...
	) = split( /\|\~\|/, $gdb::webUtil::frmData{parms} );
	...
	my $status;
	if ($change) {
		my $rc = gdb::oracle::dbupdatecuratedInfo("update curated set $change moddate=sysdate,moduser='$gdb::webUtil::username' where expid=$gdb::webUtil::frmData{id}");
		$status = ($rc) ? qq{<font color="red">* * * Error updating!! * * *</font>} : qq{<font color="green">* * * Change successfull. * * *</font>};
	} else {
		$status = qq{<font color="red">* * * No changes made! * * *</font>};
	}
...
sub expdata {
	...
	my $selid         = $gdb::webUtil::frmData{id};
	my $dbsampInfoRef = gdb::oracle::dbsampInfo($selid);
	my %dbsampInfo    = %$dbsampInfoRef;

	my $dbchannelCountsRef = gdb::oracle::dbchannelCounts();
	my %dbchannelCounts    = %$dbchannelCountsRef;

	my $dbcolumnPosNameRef = gdb::oracle::dbcolumnPosName($selid);
	...
	for my $i ( sort { $a <=> $b } keys %dbsampInfo ) {
		...
		$numchannel = $gdb::webUtil::frmData{newchan} if $gdb::webUtil::frmData{newchan};	#changed default channel
...
qq{ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Plot Type:</b> <input type="radio" id="plotma$selid" name="ptype" value="maplot" $machk>M/A &nbsp; <input type="radio" id="plotmb$selid" name="ptype" value="mbplot" $mbchk>M/Loc &nbsp; <input type="radio" id="plotxy$selid" name="ptype" value="xyplot" $xychk>X/Y\n};
	...
	if (%c1data) {
		my @RMAfiles = <$gdb::util::datapath/$dbsampInfo{0}{accession}/*.RMA>;    #see if we have RMA files
		if (@RMAfiles) {
...
sub sel1Plot {
	...
	my $testgenomeacc = '';
	my $cntlgenomeacc = '';
#	my $testgenomeacc = $gdb::webUtil::frmData{testgenome};
#	my $cntlgenomeacc = $gdb::webUtil::frmData{cntlgenome};

	#split selected test samples and put into hash
	my @testArr = split( /\,/, $gdb::webUtil::frmData{testname} );
	my %testsamp;
	...
	my @cntlArr = split( /\,/, $gdb::webUtil::frmData{cntlname} );
	my %cntlsamp;
...
#	my $testGenLtagsRef = gdb::oracle::dbgenomeLtags($testgenomeacc);
#	my %testGenomeLtags = %$testGenLtagsRef;

#	my $cntlGenLtagsRef = gdb::oracle::dbgenomeLtags($cntlgenomeacc);
#	my %cntlGenomeLtags = %$cntlGenLtagsRef;
	...
	my $log       = ( $gdb::webUtil::frmData{log}       =~ /true/i ) ? 1 : 0;
	my $normalize = ( $gdb::webUtil::frmData{normalize} =~ /true/i ) ? 1 : 0;
	my $antilog   = ( $gdb::webUtil::frmData{antilog}   =~ /true/i ) ? 1 : 0;
	my $userma    = ( $gdb::webUtil::frmData{userma}    =~ /true/i ) ? 1 : 0;
	my $plottype = ( $gdb::webUtil::frmData{plottype} ) ? $gdb::webUtil::frmData{plottype}    : '';
	my $datacol  = ( $gdb::webUtil::frmData{datacol} )  ? $gdb::webUtil::frmData{datacol} - 1 : '';    #subtract 1 because file is 0-based
	$datacol = 1 if $userma;

	my %savExpInfo = ();

	$savExpInfo{expid}     = $gdb::webUtil::frmData{id};
...
#	  qq{<tr><th class="thc" colspan="8">Test($avgTestsname) - $gdb::util::gnom{$gdb::util::gnomacc{$savExpInfo{testgenome}}}{sname} / Control($avgCntlsname) - $gdb::util::gnom{$gdb::util::gnomacc{$savExpInfo{cntlgenome}}}{sname}</th></tr>\n},
	  qq{<tr>},
	...
	my ( $plotFile, $pmap ) = gdb::plot::createExpPlot( $plottype, $stddev, $cntlgenomeacc, \%plotTestdata, \%plotCntldata );
	print qq{<tr><td class="tdc" colspan="8"><img alt="" src="/tmpimage/$plotFile" border="1" usemap="#$gdb::webUtil::frmData{id}"></td></tr>}, qq{<map name="$gdb::webUtil::frmData{id}">$pmap</map>\n},
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$gdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=Test($avgTestsname) / Control($avgCntlsname)','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
	  qq{</td></tr>\n},
	  qq{</table>\n};
	...
	if ( $gdb::webUtil::useracclevel > 2 ) {
		...
		print qq{<br>},
		  ...
		  qq{<td><input class="small" type="text" size="112" maxlength="250" id="expname$gdb::webUtil::frmData{id}" value="$expname"></td>},
qq{<td onmouseover="this.style.cursor='pointer';return overlib('Swap ExpName conditions. (Test / Control)');" onmouseout="return nd();"><input class="ebtn" type="button" value="Swap Conditions" onclick="swapcond($gdb::webUtil::frmData{id})"></td>},
		  qq{</tr>},
		  qq{<tr>},
		  qq{<td class="tdr" valign="top"><b>Infomation:</b></td>\n},
		  qq{<td class="tdl"><textarea class="small" id="info$gdb::webUtil::frmData{id}" rows="3" cols="110"></textarea></td>},
		  qq{</tr>},
		  qq{<tr>},
		  qq{<td></td>\n},
		  qq{<td><input class="ebtn" type="button" value="Save Experiment" onclick="savExptoDB(event,$gdb::webUtil::frmData{id},$gdb::webUtil::frmData{id})"></td>\n},
		  qq{</tr>},
		  qq{</table>};

		$savExpInfo{dataFilename} = $dataFilename;
		gdb::webUtil::putSessVar( 'savExpInfo', \%savExpInfo );
	}
}
...
sub SampleCorrelation {
	...
	for my $samp1 ( sort keys %corrData ) {
		...
		for my $samp2 ( sort keys %corrData ) {
			my $corr = gdb::plot::stat_correlation( $corrData{$samp1}, $corrData{$samp2} );
...
#	my $cntlgenomeacc = $gdb::webUtil::frmData{cntlgenome};
	...
	my @samples = split( /\,/, $gdb::webUtil::frmData{sampname} );
	my %samp;
...
#	my %genomeLtags = %$genomeLtagsRef;
	...
	my $antilog   = ( $gdb::webUtil::frmData{antilog}   =~ /true/i ) ? 1 : 0;
	my $plottype = ( $gdb::webUtil::frmData{plottype} ) ? $gdb::webUtil::frmData{plottype}     : '';
	my $testcol  = ( $gdb::webUtil::frmData{testcol} )  ? $gdb::webUtil::frmData{testcol} - 1  : '';    #subtract 1 because file is 0-based
	my $testbkgd = ( $gdb::webUtil::frmData{testbkgd} ) ? $gdb::webUtil::frmData{testbkgd} - 1 : '';
	my $cntlcol  = ( $gdb::webUtil::frmData{cntlcol} )  ? $gdb::webUtil::frmData{cntlcol} - 1  : '';
	my $cntlbkgd = ( $gdb::webUtil::frmData{cntlbkgd} ) ? $gdb::webUtil::frmData{cntlbkgd} - 1 : '';
...
#		  qq{<tr><th class="thc" colspan="8">$sample{$bioassays_id}{sampid} - $gdb::util::gnom{$gdb::util::gnomacc{$savExpInfo{cntlgenome}}}{sname}</th></tr>\n},
		  qq{<tr>},
		  qq{<th class="thc">DATA</th>},
		  qq{<th class="thc">NO DATA</th>},
		  qq{<th class="thc">TOTAL</th>},
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$gdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=$sample{$bioassays_id}{sampid}','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
		  qq{</td></tr>\n},
		  qq{</table>\n};

		if ( ( scalar keys %sample ) == 1 and $gdb::webUtil::useracclevel > 2 ) {
...
#		  qq{<tr><th class="thc" colspan="8">Average ($avgsname) - $gdb::util::gnom{$gdb::util::gnomacc{$savExpInfo{cntlgenome}}}{sname}</th></tr>\n},
		  qq{<tr>},
		  qq{<th class="thc">DATA</th>},
		  qq{<th class="thc">NO DATA</th>},
		  qq{<th class="thc">TOTAL</th>},
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$gdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=Average ($avgsname)','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
		  qq{</td></tr>\n},
		  qq{</table>\n};
...
sub viewPlotData {
	...
	print qq{<html><head><title>$gdb::webUtil::frmData{samp} Plot Data</title>\n},
	  qq{<link rel="stylesheet" type="text/css" href="/web/css/main.css">\n},
	  qq{<script type="text/javascript" src="/web/js/sorttable2.js"></script>\n},
	  qq{</head><body>\n};

	my $opened = open( FILE, "/run/shm/$gdb::webUtil::frmData{fname}" );
	if ($opened) {
		my @data = <FILE>;
		close(FILE);
		...
		print qq{<pre>\n},
		  qq{<b>$gdb::webUtil::frmData{samp}</b>\n},
...
sub savExptoDB {
	...
	my $savExpInfoRef = gdb::webUtil::getSessVar( 'savExpInfo' );

	my $rc = gdb::oracle::dbsaveExptoDB($savExpInfoRef);
	if ($rc) {
	...
	gdb::webUtil::putSessVar( 'savExpInfo', '' );    #empty after save
}
