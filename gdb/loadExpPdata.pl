...
sub c1data {
	...
	for my $bioassays_id ( sort keys %testsample ) {
		...
		my $testfile =
		  ($userma)
		  ? "/gdb/geo/accessions/$testsample{$bioassays_id}{accession}/$testsample{$bioassays_id}{sampid}.RMA"
		  : "/gdb/geo/accessions/$testsample{$bioassays_id}{accession}/$testsample{$bioassays_id}{fname}";
	...
	for my $bioassays_id ( sort keys %cntlsample ) {
		...
		my $cntlfile =
		  ($userma)
		  ? "/gdb/geo/accessions/$cntlsample{$bioassays_id}{accession}/$cntlsample{$bioassays_id}{sampid}.RMA"
		  : "/gdb/geo/accessions/$cntlsample{$bioassays_id}{accession}/$cntlsample{$bioassays_id}{fname}";
	...
	my $geneLocRef = gdb::oracle::dbgetGeneLoc($cntlgenomeacc);	#genome gene and start
	my %geneLoc    = %$geneLocRef;
	
	my $NSgeneLocRef = gdb::oracle::dbgetNSgeneLoc($cntlgenomeacc);
	...
	for my $id_ref ( sort keys %plotCntldata ) {
		...
		foreach my $ltag (@ltags) {
			...
			my $pdata_id = gdb::oracle::dbgetPdataNextSeq();
