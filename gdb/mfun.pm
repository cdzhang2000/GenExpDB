...
sub displayMultifun {
	...
	my ( $dbMFunSelRef, $midOrderRef ) = gdb::oracle::dbMFunSel();
	...
	for my $mid (@midOrder) {
		if ( $dbMFunSel{top}{$mid}{pid} == 0 ) {
			if ( exists $dbMFunSel{sub}{$mid} ) {
				print
				  qq{<img id="sign$mid" src="$gdb::util::webloc/web/plus.gif" onclick="expand('$mid');" alt="" onmouseover="this.style.cursor='pointer';return overlib('expand');" onmouseout="return nd();">\n};
			} else {
				print qq{ &nbsp;&nbsp;&nbsp;\n};
			}
