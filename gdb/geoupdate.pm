...
sub geoUpdateMain {
	...
	my $curatedRef = gdb::oracle::dbgetCurated();
...
qq{<td class="tdl">&nbsp;&nbsp;&nbsp;<input class="ebtn" type="button" name="testcntl" value="Fetch/Update GEO accessions" onclick="geo();" onmouseover="return overlib('Retrieve new GEO accessions');" onmouseout="return nd();">\n},
	...
	for my $acc ( sort { substr( $a, 3 ) <=> substr( $b, 3 ) } keys %curated ) {
		...
		print qq{<img id="esign$acc" src="$gdb::util::webloc/web/plus.gif" onclick="geoEdit('$acc');" alt="" onmouseover="this.style.cursor='pointer';return overlib('click to edit record');" onmouseout="return nd();">\n};
...
sub editGeoRec {
	...
	my $acc        = $gdb::webUtil::frmData{acc};
	my $curatedRef = gdb::oracle::dbgetCurated($acc);
	my %curated    = %$curatedRef;
	...
	print qq{<table cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
qq{<td valign="top"><a class="exmp" onclick="geoEdit('$gdb::webUtil::frmData{acc}');" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
...
sub saveGeoRec {
	...
	my ( $inpStatus, $inpPmid, $inpStrain, $inpSubStrain, $inpInfo ) = split( /\|\~\|/, $gdb::webUtil::frmData{parms} );
	...
	if ( !%chg ) {
		...
		my $rc = gdb::oracle::dbsavegeoEdit( $sql, \@data );
	...
	$curatedRef = gdb::oracle::dbgetCurated();
...
sub geoFetch {
	...
	my $numadded = gdb::oracle::dbputgeoUpdate( \%gse ) if %gse;
