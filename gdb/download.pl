...
sub downloaddata {
	...
	if ( $accCnt == 1 ) {
		...
		unless ( $zip->writeToFileNamed('/run/shm/OUGenExpDB.zip') == AZ_OK ) {
			die 'write error';
		}

		my $filesize = -s '/run/shm/OUGenExpDB.zip';
		...
		open( FILE, "/run/shm/OUGenExpDB.zip" );
		binmode FILE;
		print while <FILE>;
		close(FILE);
	}
...
sub downloadplotdata {
	...
	my $opened = open( FILE, "/run/shm/$frmData{fname}" );
	my @data = <FILE>;
	close(FILE);
	...
	my $filesize = -s "/run/shm/$dataFilename";
	...
	open( FILE, "/run/shm/$dataFilename" );
	binmode FILE;
	print while <FILE>;
	close(FILE);
