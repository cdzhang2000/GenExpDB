...
sub displayAnnotation {
	...
	if ( $gdb::webUtil::frmData{ginfo} and $gdb::webUtil::frmData{ginfo} =~ /^annotation/ ) {
		#ajax call
		...
		if ( $parms->{annotation} ) {
			print qq{<div class="mn2" style="border-top:1px solid #C3CCD3;"><span onclick="da('annotation');" onmouseover="mm(this,'annotation');" onmouseout="return nd();"><img id="annotationsign" src="$gdb::util::webloc/web/minus.gif" alt=""> Annotation</span></div>};
			print qq{<div class="showrec" id="annotation">};
			annotation();
			print qq{</div>\n};
		} else {
			print qq{<div class="mn2" style="border-top:1px solid #C3CCD3;"><span onclick="da('annotation');" onmouseover="mm(this,'annotation');" onmouseout="return nd();"><img id="annotationsign" src="$gdb::util::webloc/web/plus.gif" alt=""> Annotation</span></div>};
...
sub annotation {
	...
	for my $i ( sort { $a <=> $b } keys %qryltag ) {
		...
		my $dbannotRec = gdb::oracle::dbannotInfo($ltag, $genomeacc);
		my %dbannot    = %$dbannotRec;

		next if ( !$dbannot{locus_tag} );    #ltag not found
		
		my $operon = gdb::oracle::dbgetoperon( $dbannot{locus_tag} );

		my $dbregulatorsRec = gdb::oracle::dbgetregulators( $dbannot{locus_tag} );
		my %dbregulators    = %$dbregulatorsRec;

		my $dbsigmaRec = gdb::oracle::dbgetsigma( $dbannot{locus_tag} );
		my %dbsigma    = %$dbsigmaRec;

		my $dbmfunLevelsRec = gdb::oracle::dbgetmfunLevels( $dbannot{locus_tag} );
		my %dbmfunLevels    = %$dbmfunLevelsRec;

		my $dbpathwaysRec = gdb::oracle::dbgetpathways( $dbannot{gene} );
		my %dbpathways    = %$dbpathwaysRec;

		my $metalink = gdb::webUtil::metalinks($dbannotRec);
...
sub genesInOperon {
	...
	my $dboperonRec = gdb::oracle::dbgetgenesInOperon( $gdb::webUtil::frmData{sval} );
	my %dboperon    = %$dboperonRec;
	my $size        = scalar keys %dboperon;

	print qq{<hr>\n}, qq{<a class="exmp" onclick="sh('supp$gdb::webUtil::frmData{ltag}');" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a>\n},

	  qq{<div style="margin-left:20px;">\n},

	  qq{<input type="hidden" name="annottype" value="Annotation-Operon">\n},
	  qq{<input type="hidden" name="annotqry" value="$gdb::webUtil::frmData{sval}">\n},
	  qq{<table>\n},
	  qq{<tr>\n},
	  qq{<td colspan="4"><input class="ebtn" type="submit" name="qryannot" value="Query Selected"></td>\n},
	  qq{</tr>\n},
	  qq{<tr>\n},
	  qq{<td colspan="4">Operon: $gdb::webUtil::frmData{sval} ($size genes)</td>\n},
	  qq{</tr>\n},
	  qq{<tr>\n},
...
sub genesInRegulon {
	...
	my $dbregulonRec = gdb::oracle::dbgetgenesInRegulon( $gdb::webUtil::frmData{sval} );
	...
	print qq{<hr>\n}, qq{<a class="exmp" onclick="sh('supp$gdb::webUtil::frmData{ltag}');" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a>\n},
	  ...
	  qq{</tr>\n},
	  qq{<tr>\n},
	  qq{<td colspan="4">Regulon: $gdb::webUtil::frmData{sval} ($size genes)</td>\n},
	  qq{</tr>\n},
	  qq{<tr>\n},
qq{<th class="thc" onmouseover="return overlib('Select/Unselect all');" onmouseout="return nd();"><input id="ckallid" type="checkbox" name="ckallid" onclick="ckall(this,'qrySelected');"></th>\n},
	  qq{<th class="thc">GENE</th>\n},
...
sub genesInSigma {
	...
	my $dbsigmaRec = gdb::oracle::dbgetgenesInSigma( $gdb::webUtil::frmData{sval} );
	my %dbsigma    = %$dbsigmaRec;
	...
	print qq{<hr>\n}, qq{<a class="exmp" onclick="sh('supp$gdb::webUtil::frmData{ltag}');" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a>\n},
	  ...
	  qq{</tr>\n},
	  qq{<tr>\n},
	  qq{<td colspan="4">Sigma: $gdb::webUtil::frmData{sval} ($size genes)</td>\n},
	  qq{</tr>\n},
	  qq{<tr>\n},
qq{<th class="thc" onmouseover="return overlib('Select/Unselect all');" onmouseout="return nd();"><input id="ckallid" type="checkbox" name="ckallid" onclick="ckall(this,'qrySelected');"></th>\n},
	  qq{<th class="thc">GENE</th>\n},
...
sub genesInMfun {
	...
	my $dbmfunRec = gdb::oracle::dbgetgenesinMfun( $gdb::webUtil::frmData{sval} );
	my %dbmfun    = %$dbmfunRec;
	...
	print qq{<hr>\n}, qq{<a class="exmp" onclick="sh('supp$gdb::webUtil::frmData{ltag}');" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a>\n},
	  ...
	  qq{</tr>\n},
	  qq{<tr>\n},
	  qq{<td colspan="3">Multifun: $gdb::webUtil::frmData{sval} ($size genes)</td>\n},
	  qq{</tr>\n},
	  qq{<tr>\n},
qq{<th class="thc" onmouseover="return overlib('Select/Unselect all');" onmouseout="return nd();"><input id="ckallid" type="checkbox" name="ckallid" onclick="ckall(this,'qrySelected');"></th>\n},
	  qq{<th class="thc">LOCUSTAG</th>\n},
...
sub genesInPway {
	...
	my $dbpwayRec = gdb::oracle::dbgetgenesinPway( $gdb::webUtil::frmData{sval} );
	my %dbpway    = %$dbpwayRec;
	...
	print qq{<hr>\n}, qq{<a class="exmp" onclick="sh('supp$gdb::webUtil::frmData{ltag}');" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a>\n},
	  ...
	  qq{</tr>\n},
	  qq{<tr>\n},
	  qq{<td colspan="3">Pathway: $gdb::webUtil::frmData{sval} ($size genes)</td>\n},
	  qq{</tr>\n},
	  qq{<tr>\n},
qq{<th class="thc" onmouseover="return overlib('Select/Unselect all');" onmouseout="return nd();"><input id="ckallid" type="checkbox" name="ckallid" onclick="ckall(this,'qrySelected');"></th>\n},
	  qq{<th class="thc">GENE</th>\n},
...
sub displayFullqry {
	...
	my $fullqryRef = gdb::webUtil::getSessVar( 'fullqry' );
