...
sub heatmap {
	...
	testgdb::webUtil::putSessVar( 'acchm',   '' );    #clear
	...
	my $genomeacc = $testgdb::util::gnom{$parms->{genome}}{acc};
	...
	if ( $qryCnt > $parms->{dnum} ) {
		testgdb::webUtil::putSessVar( 'fullqry', \@newqry );
	...
	print "<hr>";
	print qq{<div class="small">Genome: <b>$testgdb::util::gnom{$parms->{genome}}{lname}</b></div>\n};
	...
	my $dbexpInfoRef = testgdb::oracle::dbgetExpInfo( );
	my %dbexpInfo = %$dbexpInfoRef;

	#get the experiment count
	my $expCount = 0;
	for my $i ( keys %dbexpInfo ) {
		#next if ($dbexpInfo{$i}{cntlgenome} !~ /$testgdb::util::gnom{$parms->{genome}}{acc}/i);
		$expCount++;
	}
	...
	my %genrel;
	if ($parms->{genrel} =~ /all/) {
		for my $i ( keys %testgdb::util::gnom ) {
			$genrel{ $testgdb::util::gnom{$i}{acc} } = 1;
		}
	}else{
		my @tmp = split( /~/, $parms->{genrel} );
		foreach my $t1 (@tmp) {
			$genrel{ $testgdb::util::gnom{$t1}{acc} } = 1;
		}
	}
	...
	foreach my $qry (@newqry) {
		my ( $tags, $olt, $ns ) = testgdb::oracle::qryNS( $qry, $genomeacc );
		my %tags = %$tags;
		my %olt = %$olt;
		my %ns   = %$ns;
		...
		if ( ! %tags ) {    #no ltag found in reference genome, but we may have related
			...
			if (%ns) {
				...
				for my $ltag ( sort { lc($a) cmp lc($b) } keys %ns ) {
					...
					push @ltags, $ltag;
					print qq{<a class="small" onclick="ckqry('$ltag');" onmouseover="this.style.cursor='pointer';return overlib('Query for $testgdb::util::gnom{$testgdb::util::gnomacc{$ns{$ltag}}}{sname}:$ltag');" onmouseout="return nd();">$ltag</a>&nbsp;&nbsp; };
				}
				print qq{</div>\n};
			}
			next;
		...
		my $dbgene = testgdb::oracle::dbgetGene($locusTag, $genomeacc);
		...
		my $dbHmDataRef = testgdb::oracle::dbgetheatmapData( \@ltags );
		...
		for my $i ( sort { $a <=> $b } keys %dbexpInfo ) {
			...
			my $genome = '';
#			my $genome = $testgdb::util::gnom{$testgdb::util::gnomacc{$dbexpInfo{$i}{cntlgenome}}}{sname};
	...
	if (%hminfo) {    #we have data, display heapmap
		...
		print
		  ...
		  qq{</td></tr>\n},
		  qq{<tr><td class="tdc"><img alt="" src="$testgdb::util::webloc/web/$legendFile" border="0" onmouseover="return overlib('Color legend: Log2 expression ratio of test/control');" onmouseout="return nd();"></td></tr>\n},
		  qq{</table>\n};
		...
		testgdb::webUtil::putSessVar( 'parms', $parms );
		testgdb::webUtil::putSessVar( 'acchm', \%acchm );
		...
		for my $i ( sort { $a <=> $b } keys %hminfo ) {
			...
			##plot
			if ( $testgdb::webUtil::frmData{replot} ) {
				( $testgdb::webUtil::frmData{id}, $testgdb::webUtil::frmData{selGene}, $testgdb::webUtil::frmData{nsd} ) = split( /~/, $testgdb::webUtil::frmData{replot} );
				$testgdb::webUtil::frmData{hmcnt} = $i;
				$testgdb::webUtil::frmData{qryall}  = $qryall;
				print qq{<div id="hmplot$i"><div id="pdiv$testgdb::webUtil::frmData{id}">\n};
				testgdb::plot::scatterPlot();
				print qq{</div></div>\n};
			} else {
	...
	testgdb::webUtil::putSessVar( 'qryltag',   \%ltags );
}
...
sub ckQryLoc {
	...
	foreach my $qry (@qryVal) {
		if ( $qry =~ /^[+-]?\d+$/ ) {
			...
			#numeric query
			my $result = testgdb::oracle::dbQryGenomeLocation( $qry, $genomeacc );
