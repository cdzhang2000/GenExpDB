...
sub scatterPlot {
	...
	my $hmcnt   = ( exists $testgdb::webUtil::frmData{hmcnt} )   ? $testgdb::webUtil::frmData{hmcnt}  : 0;
	my $expid   = ( $testgdb::webUtil::frmData{id} )      ? $testgdb::webUtil::frmData{id}      : 0;
	my $gene    = ( $testgdb::webUtil::frmData{gene} )    ? $testgdb::webUtil::frmData{gene}    : '';
	my $selGene = ( $testgdb::webUtil::frmData{selGene} ) ? $testgdb::webUtil::frmData{selGene} : '';
	my $qryall  = ( $testgdb::webUtil::frmData{qryall} )  ? $testgdb::webUtil::frmData{qryall}  : '';
	my $nsd     = ( $testgdb::webUtil::frmData{nsd} =~ /undefined/ ) ? 2 : $testgdb::webUtil::frmData{nsd};
	
	my ( $dbExpmRecRef ) = testgdb::oracle::dbgetExpmPlotInfo();    #get all experiment info
...
sub viewpdata {
	...
	my $hmcnt   = ( exists $testgdb::webUtil::frmData{hmcnt} )   ? $testgdb::webUtil::frmData{hmcnt}  : 0;
	my $expid = ( $testgdb::webUtil::frmData{expid} ) ? $testgdb::webUtil::frmData{expid}      : 0;

	my $dbPlotDataRef = testgdb::oracle::dbgetSPlotData($expid);    #get the plot data
...
sub linePlot {
	...
	my ( $plotFile, $pmap, $numExps ) = createLinePlot( $testgdb::util::gnom{$parms->{genome}}{acc}, $accessionID, $selGene, $qryall, $std );
...
sub createExpPlot {
	...
	#print "<pre>" . Dumper( @Xdata ) . "</pre>";
		
		
	#czhang	
	foreach my $expr (@Xdata) {
...
sub createScatterPlot {
	...
	my $dbPlotDataRef = testgdb::oracle::dbgetSPlotData($id);    #get the plot data
	my %dbPlotData    = %$dbPlotDataRef;
...
sub createLinePlot {
	...
	my @newVal = testgdb::heatmap::ckQryLoc($qryall, $genomeacc);
	
	my (%geneNames, %qryLtag);
	foreach my $qry (@newVal) {
		my ($geneRef, $ltagRef);	
		if ($genomeacc =~ /MG1655/) {
			( $geneRef, $ltagRef ) = testgdb::oracle::dbQueryNamespace($qry);
		}else{
			( $geneRef, $ltagRef ) = testgdb::oracle::dbQryGenomeLtags( $qry, $genomeacc );
		}
		my @ltags = @$ltagRef;
	...
	my $dbPlotDataRef = testgdb::oracle::dbgetLPlotAllData($id);      #get all line plot data
	my %dbPlotData    = %$dbPlotDataRef;

	my ( $dbaccExpmRec, $expm_orderRef ) = testgdb::oracle::dbgetAccExpm();    #get all accession experiments
