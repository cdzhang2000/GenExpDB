...
sub about {
	...
	my $operonDB  = testgdb::oracle::dbgetOprnDBdate();
	my $regulonDB = testgdb::oracle::dbgetRegDBdate();
	my $sigmaDB   = testgdb::oracle::dbgetSigDBdate();
	my $ecocyc    = testgdb::oracle::dbgetEcoCycDBdate();

	my @acc;
	for my $i ( sort { $a <=> $b } keys %testgdb::util::gnom ) {
		push @acc, $testgdb::util::gnom{$i}{acc};
	}
	my $acclist = "'" . join( "','", @acc ) . "'";
	my $genomeRef = testgdb::oracle::dbgetGenomeDBdate($acclist);
	my %genome = %$genomeRef;
	...
	print
	  ...
	  qq{<tr bgcolor="#ebf0f2"><td class="tdl"><b>Version</b></td><td class="tdl">$testgdb::webUtil::version</td></tr>\n},
	  ...
	  for my $i ( sort { $a <=> $b } keys %testgdb::util::gnom ) {
	  	my $accession = $testgdb::util::gnom{$i}{acc};
...
sub platforms {
	...
	my $dbplatformsRef = testgdb::oracle::dbplatformInfo();
	my %dbplatforms    = %$dbplatformsRef;

	my $dbpfcntRef = testgdb::oracle::dbplatformCounts();
...
sub downloadAccessions {
	...
	my $allchecked = ($testgdb::webUtil::frmData{aid}) ? 'checked' : '';
	...
	for my $i ( sort { $a <=> $b } keys %dbAccession ) {
		...
		my $checked = ( $testgdb::webUtil::frmData{aid} =~ /$id/ ) ? 'checked' : '';
		...
		print qq{<tr bgcolor="#ebf0f2">\n},
			...
			qq{<td class="tdl">},
			qq{<img id="dlesign$id" src="$testgdb::util::webloc/web/plus.gif" onclick="ckdlaccexp($id);" alt="" onmouseover="this.style.cursor='pointer';return overlib('click to display experiments');" onmouseout="return nd();"> },
...
sub showdlexpm {
	...
	my $ExpmCount  = 0;
	my $hasExp     = 0;
	my $ckaccn = ( $testgdb::webUtil::frmData{ckdlaccn} =~ /true/ ) ? 'checked' : '';    #see if accession was checked
	
	print qq{<table align="center" cellpadding="1" cellspacing="1">\n},
	  qq{<tr>\n},
	  qq{<td valign="top"><a class="exmp" onclick="ckdlaccexp($testgdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	...
	for my $id (@expm_order) {
		...
		if ( $testgdb::webUtil::frmData{id} == $dbaccExpm{$id}{expid} ) {
			...
			print qq{<tr bgcolor="#ebf0f2">\n},
			  qq{<td class="tdc"><input id="ckdlexpm$id" class="small" type="checkbox" name="ckdlexpm" value="$testgdb::webUtil::frmData{id}:$id" onclick="ckexpmt(this,'ckdlaccn','ckalldlexp','$dbaccExpm{$id}{expid}');" $ckaccn></td>\n},
...
sub accsamples {
	...
	my $dbaccSamplesRef = testgdb::oracle::dbaccSamplesInfo();	#all accessions and all samples
	my %accSamples  = %$dbaccSamplesRef;

	my ($dbexpSamplesRef, $expcntRef) = testgdb::oracle::dbexpSamplesInfo();	#active accessions experiments and samples used
	...
	for my $acc ( sort { substr( $a, 3 ) <=> substr( $b, 3 ) } keys %expSamples ) {
		for my $samp ( sort { substr( $a, 3 ) <=> substr( $b, 3 ) } keys %{$expSamples{$acc}} ) {
			for my $exp ( sort { $a cmp $b } keys %{$expSamples{$acc}{$samp}} ) {
				...
				next if ($expstrain !~ /$testgdb::util::gnom{$parms->{genome}}{acc}/i);
	...
	for my $acc ( sort { substr( $a, 3 ) <=> substr( $b, 3 ) } keys %expSamples ) {
		for my $samp ( sort { substr( $a, 3 ) <=> substr( $b, 3 ) } keys %{$expSamples{$acc}} ) {
			for my $exp ( sort { $a cmp $b } keys %{$expSamples{$acc}{$samp}} ) {
				...
				next if ($strain !~ /$testgdb::util::gnom{$parms->{genome}}{acc}/i);
	...
	for my $acc ( sort { substr( $a, 3 ) <=> substr( $b, 3 ) } keys %data ) {
		...
		for my $samp ( sort { substr( $a, 3 ) <=> substr( $b, 3 ) } keys %{$data{$acc}} ) {
			...
			for my $exp ( sort { $a cmp $b } keys %{$data{$acc}{$samp}} ) {
				my $genome = $testgdb::util::gnom{$testgdb::util::gnomacc{$data{$acc}{$samp}{$exp}}}{sname};
...
sub expgenes {
	...
	my $dbexpgenesRef = testgdb::oracle::dbexpGenesInfo();
...
sub stats {
	...
	my $dbstatsRef = testgdb::oracle::dbstatsInfo();
