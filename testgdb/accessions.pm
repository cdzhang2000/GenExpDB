...
sub displayAccessions
{
	...
	if (     $testgdb::webUtil::frmData{ginfo}
		 and $testgdb::webUtil::frmData{ginfo} =~ /^accessions/ )
	{
	...
	} else
	{
		...
		{
			print
qq{<div class="mn2" style="border-top:1px solid #C3CCD3;"><span onclick="da('accessions');" onmouseover="mm(this,'accessions');" onmouseout="return nd();"><img id="accessionssign" src="$testgdb::util::webloc/web/minus.gif" alt=""> Accessions</span></div>};
		...
		{
			print
qq{<div class="mn2" style="border-top:1px solid #C3CCD3;"><span onclick="da('accessions');" onmouseover="mm(this,'accessions');" onmouseout="return nd();"><img id="accessionssign" src="$testgdb::util::webloc/web/plus.gif" alt=""> Accessions</span></div>};
...
sub accessions
{
	...
	for my $i ( sort { $a <=> $b } keys %dbAccession )
	{
		...
		if (    ( $parms->{experiment} =~ /^Experiment/ )
			 or $parms->{foldck}
			 or $parms->{logck} )
		{
		...
		$line{$i}{cb} =
qq{<td class="tdc"><input id="ckaccn$id" class="small" type="checkbox" name="ckaccn" value="$id" onclick="ckqry('$parms->{currquery}');" $checked $disabled></td>\n};
		...
		{
			$tmpline .=
qq{<img id="esign$id" src="$testgdb::util::webloc/web/plus.gif" onclick="ckaccexp($id);" alt="" onmouseover="this.style.cursor='pointer';return overlib('click to display experiments');" onmouseout="return nd();"> };
		} else
		{
			$tmpline .= "&nbsp;&nbsp;&nbsp;&nbsp;";
		}
		$tmpline .=
qq{<a href="http://www.ncbi.nlm.nih.gov/projects/geo/query/acc.cgi?acc=$dbAccession{$i}{accession}" target="_blank" onmouseover="return overlib('query this accession in Geo (new window)');" onmouseout="return nd();">$dbAccession{$i}{accession}</a>};
		$tmpline .= qq{</td>\n};
		...
		if ( $acchm{$id} )
		{
			...
			for my $j ( sort { $a <=> $b } keys %{ $acchm{$id} } )
			{
				...
				$tmpline .=
qq{<td class="tdc" width="8" style="background-color:$acchm{$id}{$j}{ratioColor};" onclick="$acchm{$id}{$j}{click} window.scrollTo(0,0);" onmouseover="return overlib('$acchm{$id}{$j}{title}',WIDTH,400);" onmouseout="return nd();">$acchm{$id}{$j}{hm}</td>}
				  if $acchm{$id}{$j};
				$ic++;
			}
			$tmpline .= qq{</tr>\n};
		...
		#INFO
		$line{$i}{info} =
qq{<td class="tdc"><a onclick="accinfo('accinfo',$id);" onmouseover="this.style.cursor='pointer';return overlib('Accession Information');" onmouseout="return nd();">info</a></td>\n};

		#INSTITUTION_[PI]
		$tmpline = '';
		my $institution =
		  ( $dbAccession{$i}{institution} )
		  ? $dbAccession{$i}{institution}
		  : $dbLabOrgRec{ $dbAccession{$i}{eid} };
		$institution =
		  ( $dbAccession{$i}{pi} )
		  ? "$institution [<b>$dbAccession{$i}{pi}</b>]"
		  : $institution;
		if ( length($institution) > 70 )
		{
			$tmp = substr $institution, 0, 67;
			$tmpline .=
qq{<td class="tdl" onmouseover="return overlib('$institution');" onmouseout="return nd();">$tmp<b>...</b></td>\n};
		} else
		{
		...
		#REFERENCE
		my @pmA;
		@pmA = split( /,/, $dbExpmRefRec{$id} )
		  if $dbExpmRefRec{$id};    #get geo pubmed
		@pmA = split( /,/, $dbAccession{$i}{pmid} )
		  if $dbAccession{$i}{pmid};    #get curated pubmed
		@pmA = sort { $a <=> $b } @pmA;
		my $pubmed = join( ',', @pmA );
		my $reference =
		  ( $dbAccession{$i}{author} )
		  ? $dbAccession{$i}{author} . ', et al, '
		  : '';
		my $pmS = '';
		...
		foreach my $pm (@pmA)
		{
			...
			$pm =~ s/\s$//g;
			$pmS .=
qq{<a href="http://www.ncbi.nlm.nih.gov/pubmed/$pm" target="_blank" onmouseover="return overlib('query PubMed (new window)');" onmouseout="return nd();">$pm</a>, };
		}
		$pmS =~ s/, $//;
		...
		if ( length($title) > 50 )
		{
			...
			$tmpline .=
qq{<td class="tdl" onmouseover="return overlib('$title');" onmouseout="return nd();">$tmp<b>...</b></td>\n};
		} else
		{
		...
		$tmpline = '';
		my $cdt =
		  ( $dbAccession{$i}{designtype} )
		  ? $dbAccession{$i}{designtype} . ';'
		  : '';
		my $cts =
		  ( $dbAccession{$i}{timeseries} )
		  ? $dbAccession{$i}{timeseries} . ';'
		  : '';
		my $ctm =
		  ( $dbAccession{$i}{treatment} )
		  ? $dbAccession{$i}{treatment} . ';'
		  : '';
		my $cgc =
		  ( $dbAccession{$i}{growthcond} )
		  ? $dbAccession{$i}{growthcond} . ';'
		  : '';
		my $cmd =
		  ( $dbAccession{$i}{modification} )
		  ? $dbAccession{$i}{modification} . ';'
		  : '';
		...
		if ( length($design) > 40 )
		{
			...
			$tmpline .=
qq{<td class="tdl" onmouseover="return overlib('$design');" onmouseout="return nd();">$tmp<b>...</b></td>\n};
		} else
		{
		...
		$tmpline = '';
		my $curArrDsgn =
		  ( $dbAccession{$i}{arraydesign} )
		  ? $dbAccession{$i}{arraydesign}
		  : '';
		my $expArrDsgn =
		  ( $dbPlatformDesignRec{ $dbAccession{$i}{eid} }{ad} )
		  ? $dbPlatformDesignRec{ $dbAccession{$i}{eid} }{ad}
		  : '';
		...
		if ( length($arrDsgn) > 40 )
		{
			...
			$tmpline .=
qq{<td class="tdl" onmouseover="return overlib('$arrDsgn');" onmouseout="return nd();">$tmp<b>...</b></td>\n};
		} else
		{
		...
		#STRAIN
		my $strain =
		  ( $dbAccession{$i}{strain} ) ? $dbAccession{$i}{strain} : '';
		my $substrain =
		  ( $dbAccession{$i}{substrain} ) ? $dbAccession{$i}{substrain} : '';
		$strain = ($substrain) ? "$strain ($substrain)" : $strain;
		$line{$i}{genome} = qq{<td class="tdl">$strain</td>\n};

		#czhang GEO matched organism
		my $organism =
		  ( $dbAccession{$i}{organism} ) ? $dbAccession{$i}{organism} : '';
		...
		#PLATFORM
		my $pfd =
		  ( $dbPlatformDesignRec{ $dbAccession{$i}{eid} }{gpl} )
		  ? $dbPlatformDesignRec{ $dbAccession{$i}{eid} }{gpl}
		  : '';
		my $gplS = '';
		my @gplA = split( /,/, $pfd );
		...
		{
			$gplS .=
qq{<a href="http://www.ncbi.nlm.nih.gov/projects/geo/query/acc.cgi?acc=$gpl" target="_blank" onmouseover="return overlib('query this platform in Geo (new window)');" onmouseout="return nd();">$gpl</a>, };
		}
		$gplS =~ s/, $//;
		...
		#czhang added MDUSER
		my $moduser =
		  ( $dbAccession{$i}{moduser} ) ? $dbAccession{$i}{moduser} : '';
	...
	my $accHead =
	  ( $col =~ /accession/ )
	  ? qq{ACCESSION<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "ACCESSION";
	my $expsHead =
	  ( $col =~ /exps/ )
	  ? qq{EXPS<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "EXPS";
	my $institutionHead =
	  ( $col =~ /institution/ )
	  ? qq{INSTITUTION_[PI]<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "INSTITUTION_[PI]";
	my $referenceHead =
	  ( $col =~ /reference/ )
	  ? qq{REFERENCE<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "REFERENCE";
	my $titleHead =
	  ( $col =~ /title/ )
	  ? qq{ACCESSION TITLE<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "ACCESSION TITLE";
	my $experimentHead =
	  ( $col =~ /experiment/ )
	  ? qq{EXPERIMENT DESIGN<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "EXPERIMENT DESIGN";
	my $arrayHead =
	  ( $col =~ /array/ )
	  ? qq{ARRAY DESIGN<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "ARRAY DESIGN";
	my $genomeHead =
	  ( $col =~ /genome/ )
	  ? qq{SAMPLE SOURCE<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "SAMPLE SOURCE";

	#czhang
	my $organismHead =
	  ( $col =~ /organism/ )
	  ? qq{ORGANISM<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "ORGANISM";
	...
	my $platformHead =
	  ( $col =~ /platform/ )
	  ? qq{PLATFORM<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "PLATFORM";

	#czhang
	my $moduserHead =
	  ( $col =~ /moduser/ )
	  ? qq{MODUSER<img id="accessionssort" src="$testgdb::util::webloc/web/$arrow.gif" alt="">}
	  : "MODUSER";
...
qq{<th><span class="shc" onclick="sm('accession','$dir');" onmouseover="return overlib('GEO Accession ID, click to sort');" onmouseout="return nd();">$accHead</span></th>\n};
	...
	print
qq{<th onmouseover="return overlib('Heatmap for gene $parms->{currquery}');" onmouseout="return nd();">HEATMAP</th>\n}
	  if ( keys(%acchm) );
	print
...
qq{<th><span class="shc" onclick="sm('moduser','$dir');" onmouseover="return overlib('Moduser, click to sort');" onmouseout="return nd();">$moduserHead</span></th>\n},
	...
	{
		print $line{$i}{tr}, $line{$i}{cb}, $line{$i}{accession};
		print $line{$i}{heatmap} if ( keys(%acchm) );
		print $line{$i}{exps}, $line{$i}{info}, $line{$i}{institution},
		  $line{$i}{reference}, $line{$i}{title}, $line{$i}{experiment},
		  $line{$i}{array},     $line{$i}{genome},
		...
		if ( $sexp{ $line{$i}{id} } )
		{
			...
			print
qq{<tr><td colspan="12"><div class="showrec" id="expmt$line{$i}{id}">};
			accExperiments();
			print qq{</div></td></tr>\n};
		} else
		{
			print
qq{<tr><td colspan="12"><div class="hidden" id="expmt$line{$i}{id}"></div></td></tr>\n};
		}

		#hidden experiments info
		print
qq{<tr><td colspan="12"><div class="hidden" id="accinfo$line{$i}{id}"></div></td></tr>\n};
	}
	print
...
sub accExperiments
{
	...
	my ( $dbaccExpmRec, $expm_orderRef ) =
	  testgdb::oracle::dbgetAccExpm()
	  ;    #get all accession experiments so the OUID will be correct
...
qq{<td valign="top"><a class="exmp" onclick="ckaccexp($testgdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	...
	for my $id (@expm_order)
	{
		...
		{
			next
			  if ( $dbaccExpm{$id}{cntlgenome} !~
				   /$testgdb::util::gnom{$parms->{genome}}{acc}/i );
...
qq{<th class="thc" onmouseover="return overlib('Select/Unselect all');" onmouseout="return nd();"><input id="ckallexp$dbaccExpm{$id}{expid}" type="checkbox" name="ckallid" onclick="ckallexpmt(this,'ckaccn','ckexpm',$dbaccExpm{$id}{expid});ckqry('$parms->{currquery}');" $ckaccn></th>\n},
			...
			if ( $parms->{expmtid}
				 and ( index( $parms->{expmtid}, $dbaccExpm{$id}{expid} ) >= 0 )
			  )
			{
				$ckaccn =
				  ( index( $parms->{expmtid}, $id ) < 0 ) ? '' : 'checked';
			}

			my $testgenome =
			  ( $dbaccExpm{$id}{testgenome} )
			  ? $testgdb::util::gnom{ $testgdb::util::gnomacc{ $dbaccExpm{$id}
					  {testgenome} } }{sname}
			  : '';
			my $cntlgenome =
			  ( $dbaccExpm{$id}{cntlgenome} )
			  ? $testgdb::util::gnom{ $testgdb::util::gnomacc{ $dbaccExpm{$id}
					  {cntlgenome} } }{sname}
			  : '';
...
qq{<td class="tdc"><input id="ckexpm$id" class="small" type="checkbox" name="ckexpm" value="$testgdb::webUtil::frmData{id}:$id" onclick="ckexpmt(this,'ckaccn','ckallexp','$dbaccExpm{$id}{expid}');ckqry('$parms->{currquery}');" $ckaccn></td>\n},
	...
	{
		print
qq{<tr><td class="tdl" colspan="3">Recs: <b>$ExpmCount</b></td></tr>\n};
	} else
	{
...
qq{<td class="tdc"><a onclick="nd();accinfo('curated',$testgdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('GenExpDB Curated Info');" onmouseout="return nd();">Curated</a> &nbsp; | &nbsp;</td>\n}
	  if (     $testgdb::webUtil::useracclevel
		   and $testgdb::webUtil::useracclevel > 2 );
...
sub providers
{
	...
	my $dbexpProvidersRef =
	  testgdb::oracle::dbexpProviders( $testgdb::webUtil::frmData{id} );
...
sub summary
{
	...
	my $dbexpSummaryRef =
	  testgdb::oracle::dbexpSummary( $testgdb::webUtil::frmData{id} );
...
sub expdesign
{
	...
	my $dbexpDesignRef =
	  testgdb::oracle::dbexpDesign( $testgdb::webUtil::frmData{id} );
...
sub arraydesign
{
	...
	my $dbarrayDesignRef =
	  testgdb::oracle::dbarrayDesign( $testgdb::webUtil::frmData{id} );
...
sub sampinfo
{
	...
	my $dbsampInfoRef =
	  testgdb::oracle::dbsampInfo( $testgdb::webUtil::frmData{id} );
...
qq{<td class="tdc" bgcolor="#ebf0f2"><a onclick="window.open('$ENV{REQUEST_URI}?ajax=accinfo&accinfo=viewRawData&view=$dbsampInfo{$colsamp{$col}{$row}}{fname}&accession=$dbsampInfo{$colsamp{$col}{$row}}{accession}&sname=$dbsampInfo{$colsamp{$col}{$row}}{samid}&id=$dbsampInfo{$colsamp{$col}{$row}}{bioassays_id}', '_blank')" onmouseover="this.style.cursor='pointer';return overlib('View raw data (new window)');" onmouseout="return nd();">view</a></td>\n},
		...
		print
qq{<tr><td colspan="$spancol"><div class="hidden" id="sampdetail$i$testgdb::webUtil::frmData{id}"></div></td></tr>\n};
		$i++;
	}
...
sub sampdetail
{
	...
	my ( $sampIDname, $dbsampDetailRef ) =
	  testgdb::oracle::dbsampDetail( $testgdb::webUtil::frmData{sampID} );
	my %dbsampDetail         = %$dbsampDetailRef;
	my $dbsampDescriptionRef =
	  testgdb::oracle::dbsampDescription( $testgdb::webUtil::frmData{sampID} );
...
qq{<td valign="top"><a class="exmp" onclick="sh('$testgdb::webUtil::frmData{id}');" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	...
	for my $name ( sort keys %dbsampDetail )
	{
		...
		{
			print
qq{<td class="tdl" bgcolor="#ebf0f2">$dbsampDetail{$name}{$channel}</td>\n};
		}
		print qq{</tr>\n};
	}
...
sub viewRawData
{
	...
	my $opened = open( FILE,
"$testgdb::util::datapath/$testgdb::webUtil::frmData{accession}/$testgdb::webUtil::frmData{view}"
	);

	print
qq{<html><head><title>$testgdb::webUtil::frmData{sname} Raw Data</title>\n},
qq{<script type="text/javascript" src="/web/js/overlib/Mini/overlib_mini.js"></script><!-- overLIB (c) Erik Bosrup -->\n},
	...
	if ($opened)
	{
		...
		my ($labelRef) =
		  testgdb::oracle::dbsampFilehead( $testgdb::webUtil::frmData{id} );
		my %label = %$labelRef;
...
qq{<input class="ebtn" type="button" value="View full table" onclick="window.location.href='$testgdb::util::url/?ajax=accinfo&accinfo=viewRawData&view=$testgdb::webUtil::frmData{view}&accession=$testgdb::webUtil::frmData{accession}&sname=$testgdb::webUtil::frmData{sname}&id=$testgdb::webUtil::frmData{id}&full=1'">\n};
		...
		foreach my $head (@col)
		{
			my $desc =
			  ( $desc[$i] )
			  ? qq{onmouseover="this.style.cursor='pointer';return overlib('$desc[$i]');" onmouseout="return nd();"}
			  : '';
...
sub expinfo
{
	...
	my ( $dbExpmRef, $expmorderRef ) =
	  testgdb::oracle::dbgetExpmInfo();    #all experiments, needed for ouid
	my %dbExpm = %$dbExpmRef;
...
qq{<td valign="top"><a class="exmp" onclick="accinfo('expinfo',$testgdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	...
	print
qq{<tr><td class="tdc" colspan="3"><input class="ebtn" type="button" value="Update" onclick="updexperiment($expid,this.form)"></td><td class="small">$status</td></tr>\n}
	...
	#for my $id ( sort { $a <=> $b } keys %dbexpInfo )
	{
		...
		my $dbcolumnNameRef =
		  testgdb::oracle::dbcolumnName( $dbexpInfo{$id}{samples} )
		  ;    #get all columns for this sample
		my %dbcolumnName = %$dbcolumnNameRef;

		my $testcolumn =
		  ( $dbcolumnName{ $dbexpInfo{$id}{testcolumn} } )
		  ? $dbcolumnName{ $dbexpInfo{$id}{testcolumn} }
		  : '';
		my $testbkgd =
		  ( $dbcolumnName{ $dbexpInfo{$id}{testbkgd} } )
		  ? $dbcolumnName{ $dbexpInfo{$id}{testbkgd} }
		  : '';
		my $testgenome =
		  ( $dbexpInfo{$id}{testgenome} )
		  ? $testgdb::util::gnom{ $testgdb::util::gnomacc{ $dbexpInfo{$id}
				  {testgenome} } }{sname}
		  : '';
		my $controlcolumn =
		  ( $dbcolumnName{ $dbexpInfo{$id}{controlcolumn} } )
		  ? $dbcolumnName{ $dbexpInfo{$id}{controlcolumn} }
		  : '';
		my $cntlbkgd =
		  ( $dbcolumnName{ $dbexpInfo{$id}{cntlbkgd} } )
		  ? $dbcolumnName{ $dbexpInfo{$id}{cntlbkgd} }
		  : '';
		my $cntlgenome =
		  ( $dbexpInfo{$id}{cntlgenome} )
		  ? $testgdb::util::gnom{ $testgdb::util::gnomacc{ $dbexpInfo{$id}
				  {cntlgenome} } }{sname}
		  : '';
		...
		print
qq{<td class="tdc"><input type="checkbox" name="deleteExp$expid$expmID" value="1"></td>\n}
		  if ( $testgdb::webUtil::useracclevel > 2 );
		print
qq{<td class="tdc"><input class="tdr" type="text" size="3" maxlength="3" name="chgexporder$expid$expmID" value="$dbexpInfo{$id}{exporder}"></td>\n}
		  if ( $testgdb::webUtil::useracclevel > 2 );

		if ( $testgdb::webUtil::useracclevel > 2 )
		{
			print
qq{<td class="tdl"><input class="tdl" type="text" size="80" maxlength="250" name="chgExpName$expid$expmID" value="$dbexpInfo{$id}{expname}"></td>\n};
		} else
		{
		...
		{
			print
qq{<td class="tdc"><input class="tdr" type="text" size="5" maxlength="9" name="chgtimepoint$expid$expmID" value="$dbexpInfo{$id}{timepoint}"></td>\n};
		} else
		{
	...
	print
qq{<tr><td class="tdc" colspan="3"><input class="ebtn" type="button" value="Update" onclick="updexperiment($expid,this.form)"></td></tr>\n}
	  if ( $expCnt > 0 and $testgdb::webUtil::useracclevel > 2 );
	print qq{</table>\n};
...
sub updexperiment
{
	...
	my $dbexpInfoRef =
	  testgdb::oracle::dbexpInfo( $testgdb::webUtil::frmData{id} );
	my %dbexpInfo = %$dbexpInfoRef;

	$testgdb::webUtil::frmData{en} =~
	  s/\x2B/\+/g;    #these were changed to get thru html post
	...
	for my $id ( sort { $a <=> $b } keys %dbexpInfo )
	{
		...
		} else
		{
			...
			$change .= "exporder=$neword,"
			  if $dbexpInfo{$id}{exporder} ne $exporder[$id];

			$dbexpInfo{$id}{expname} = '' if !$dbexpInfo{$id}{expname};
			$expname[$id] = '' if !$expname[$id];
			$change .= "expname='$expname[$id]',"
			  if $dbexpInfo{$id}{expname} ne $expname[$id];

			$dbexpInfo{$id}{timepoint} = '' if !$dbexpInfo{$id}{timepoint};
			$exptimept[$id] = '' if !$exptimept[$id];
			my $newtp = ( $exptimept[$id] ) ? $exptimept[$id] : "''";
			$change .= "timepoint=$newtp,"
			  if $dbexpInfo{$id}{timepoint} ne $exptimept[$id];

			push @allchg,
"update pexp set $change moddate=sysdate,moduser='$testgdb::webUtil::username' where id=$dbexpInfo{$id}{id}"
			  if $change;
		}
	}
	...
	if (@allchg)
	{
		...
		$status =
		  ($rc)
		  ? qq{<font color="red">* * * Error updating!! * * *</font>}
		  : qq{<font color="green">* * * Change successfull. * * *</font>};
	} else
	{
...
sub curated
{
	...
	my %dbexpDesign       = %$dbexpDesignRef;
	my $dbexpProvidersRef =
	  testgdb::oracle::dbexpProviders($id);    #org and provider
	my %dbexpProviders  = %$dbexpProvidersRef;
...
sub updcurated
{
	...
	my $dbcuratedInfoRef =
	  testgdb::oracle::dbcuratedInfo( $testgdb::webUtil::frmData{id} );
	my %dbcuratedInfo = %$dbcuratedInfoRef;
	...
	my (
		 $pi,         $institution,  $author,      $pmid,
		 $title,      $designtype,   $timeseries,  $treatment,
		 $growthcond, $modification, $arraydesign, $strain,
		 $substrain,  $info
	  )
	...
	$change .= "institution='$institution',"
	  if ( $dbcuratedInfo{institution} ne $institution );
	...
	$change .= "designtype='$designtype',"
	  if ( $dbcuratedInfo{designtype} ne $designtype );
	$change .= "timeseries='$timeseries',"
	  if ( $dbcuratedInfo{timeseries} ne $timeseries );
	$change .= "treatment='$treatment',"
	  if ( $dbcuratedInfo{treatment} ne $treatment );
	$change .= "growthcond='$growthcond',"
	  if ( $dbcuratedInfo{growthcond} ne $growthcond );
	$change .= "modification='$modification',"
	  if ( $dbcuratedInfo{modification} ne $modification );
	$change .= "arraydesign='$arraydesign',"
	  if ( $dbcuratedInfo{arraydesign} ne $arraydesign );
	$change .= "strain='$strain'," if ( $dbcuratedInfo{strain} ne $strain );
	$change .= "substrain='$substrain',"
	  if ( $dbcuratedInfo{substrain} ne $substrain );
	...
	{
		my $rc =
		  testgdb::oracle::dbupdatecuratedInfo(
"update curated set $change moddate=sysdate,moduser='$testgdb::webUtil::username' where expid=$testgdb::webUtil::frmData{id}"
		  );
		$status =
		  ($rc)
		  ? qq{<font color="red">* * * Error updating!! * * *</font>}
...
sub expdata
{
	...
	print "sub expdata :: selid=$selid <br>";
	...
	for my $i ( sort { $a <=> $b } keys %dbsampInfo )
	{
		...
		{
			$chgNumchan =
qq{&nbsp;&nbsp;&nbsp;(<a class="exmp" onclick="chgchn('2','$selid');" onmouseover="this.style.cursor='pointer';return overlib('Create experiment as a 2-channel');" onmouseout="return nd();">Run as 2-channel</a>)};
			$c1data{ $dbsampInfo{$i}{samid} }{bioassays_id} =
			  $dbsampInfo{$i}{bioassays_id};
			$c1data{ $dbsampInfo{$i}{samid} }{accession} =
			  $dbsampInfo{$i}{accession};
		...
		{
			$chgNumchan =
qq{&nbsp;&nbsp;&nbsp;(<a class="exmp" onclick="chgchn('1','$selid');" onmouseover="this.style.cursor='pointer';return overlib('Create experiment as a 1-channel');" onmouseout="return nd();">Run as 1-channel</a>)};
			$c2data{ $dbsampInfo{$i}{samid} }{bioassays_id} =
			  $dbsampInfo{$i}{bioassays_id};
			$c2data{ $dbsampInfo{$i}{samid} }{accession} =
			  $dbsampInfo{$i}{accession};
...
qq{ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Plot Type:</b> <input type="radio" id="plotma$selid" name="ptype" value="maplot" $machk>M/A &nbsp; <input type="radio" id="plotmb$selid" name="ptype" value="mbplot" $mbchk>M/Loc &nbsp; <input type="radio" id="plotxy$selid" name="ptype" value="xyplot" $xychk>X/Y\n};
	...
	{
		my @RMAfiles =
		  <$testgdb::util::datapath/$dbsampInfo{0}{accession}/*.RMA>
		  ;    #see if we have RMA files
		if (@RMAfiles)
		{
			print
qq{ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="userma$selid" value="1">Use RMA data\n};
		}
...
qq{<select class="small" id="testname$selid" size="$selsize" MULTIPLE>\n};
		...
		for my $samid (
			sort
			{
				lc $c1data{$a}{name} cmp lc $c1data{$b}{name}
				  || $c1data{$a}{name} cmp $c1data{$b}{name}
			} keys %c1data
		  )
		{    # sort sample list by sample name
			print
qq{<option value="$c1data{$samid}{bioassays_id}">$samid &nbsp; $c1data{$samid}{name}</option>\n};
		}
		print qq{</select>\n}, qq{</td>\n},
...
qq{<option value="$c1data{$samid}{bioassays_id}">$samid &nbsp; $c1data{$samid}{name}</option>\n};
		...
		for my $pos ( sort { $a <=> $b }
					keys %{ $dbcolumnPosName{ $dbsampInfo{0}{bioassays_id} } } )
		{
			next if $pos == 1;    #1 is ID_REF
			print
qq{<option value="$pos">$dbcolumnPosName{$dbsampInfo{0}{bioassays_id}}{$pos}</option>\n};
		}
		print qq{</select>\n}, qq{</td>\n}, qq{</tr>\n}, qq{</table>\n};
	}
	if (%c2data)
...
qq{<select class="small" id="sampname$selid" size="$selsize" MULTIPLE>\n};
		...
		for my $samid (
			sort
			{
				lc $c2data{$a}{name} cmp lc $c2data{$b}{name}
				  || $c2data{$a}{name} cmp $c2data{$b}{name}
			} keys %c2data
		  )
		{    # sort sample list by sample name
...
#		for my $samid (sort {$c2data{$a} cmp $c2data{$b} } keys %c2data){ # sort sample list by sample name
			...
			print
qq{<option value="$c2data{$samid}{bioassays_id}">$samid &nbsp; $c2data{$samid}{name}</option>\n};
		}
		print qq{</select>\n}, qq{</td>\n},
...
sub sel1Plot
{
	...
	{
		print
qq{<font color="red">No Platform available!  Please contact administrator.</font>};
		return;
	}

	my $log = ( $testgdb::webUtil::frmData{log} =~ /true/i ) ? 1 : 0;
	my $normalize =
	  ( $testgdb::webUtil::frmData{normalize} =~ /true/i ) ? 1 : 0;
	my $antilog = ( $testgdb::webUtil::frmData{antilog} =~ /true/i ) ? 1 : 0;
	my $userma  = ( $testgdb::webUtil::frmData{userma}  =~ /true/i ) ? 1 : 0;
	my $plottype =
	  ( $testgdb::webUtil::frmData{plottype} )
	  ? $testgdb::webUtil::frmData{plottype}
	  : '';
	my $datacol =
	  ( $testgdb::webUtil::frmData{datacol} )
	  ? $testgdb::webUtil::frmData{datacol} - 1
	  : '';    #subtract 1 because file is 0-based
	$datacol = 1 if $userma;
	...
	for my $bioassays_id ( sort keys %testsample )
	{
		...
		my $dbchannelSourceRef =
		  testgdb::oracle::dbchannelSource($bioassays_id);
...
qq{<font color="red">Cannot open Test sample file $testsample{$bioassays_id}{sampid} !!</font>};
		...
		foreach my $line (@data)
		{
			...
			my @gtags =
			  split( ":", $id_ref )
			  ; #split multiple ltags per id so we can check against genome ltags
			foreach my $gt (@gtags)
			{
...
#antilog - convert log 10 to base number, log10(num)=val  antilog== 10^val = num
			...
			#czhang
			if($dataVal ne '' and looks_like_number($dataVal)){
			push @maxval, $dataVal;
			}
...
#antilog - convert log 10 to base number, log10(num)=val  antilog== 10^val = num
	...
	{
		print
qq{<font color="red">ID refs are inconsistent between platform and soft xml !</font>\n};
		return;
	...
	{
		print
qq{<font color="red">Data does not seem to be Log values? (maximum value > 24)</font>\n};
	}
	...
	my $dataFilename =
	  writeDataToFile( $cntlgenomeacc, \%plotTestdata, \%plotCntldata );
	...
	$savExpInfo{samples} =
	  join( ',', @testSampname ) . "/" . join( ',', @cntlSampname );
...
qq{<tr><th class="thc" colspan="8">Test($avgTestsname) / Control($avgCntlsname) </th></tr>\n},
	  qq{<tr>}, qq{<th class="thc">DATA</th>}, qq{<th class="thc">NO DATA</th>},
	  qq{<th class="thc">TOTAL</th>},  qq{<th class="thc">PLATFORM</th>},
	  qq{<th class="thc">STDDEV</th>}, qq{<th class="thc">MEAN</th>},
	  qq{<th class="thc">MIN</th>},    qq{<th class="thc">MAX</th>}, qq{</tr>},
	  qq{<tr bgcolor="#EBF0F2">},      qq{<td class="tdc">$ltagCnt</td>\n},
	...
	{
		print
qq{<tr><td class="tdc" colspan="8"><font color="red">\tCounts are not equal!</font></td></tr>\n};
	}

	my ( $plotFile, $pmap ) = testgdb::plot::createExpPlot(
												 $plottype,      $stddev,
												 $cntlgenomeacc, \%plotTestdata,
												 \%plotCntldata
	);
	print
qq{<tr><td class="tdc" colspan="8"><img alt="" src="/tmpimage/$plotFile" border="1" usemap="#$testgdb::webUtil::frmData{id}"></td></tr>},
	  qq{<map name="$testgdb::webUtil::frmData{id}">$pmap</map>\n},
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$testgdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=Test($avgTestsname) / Control($avgCntlsname)','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
	...
	if ( $testgdb::webUtil::useracclevel > 2 )
	{
		...
		print qq{<br>},
		  ...
		  qq{<tr>}, qq{<td class="tdr"><b>ExpName:</b></td>},
...
sub SampleCorrelation
{
	...
	for my $samp1 ( sort keys %corrData )
	{
		...
		{
			my $corr = testgdb::plot::stat_correlation( $corrData{$samp1},
														$corrData{$samp2} );
			...
			if ( $corr > 0.9 )
			{
				...
				{
					print
					  qq{<td class="tdc"><font color="red">$corr</font></td>};
				} else
				{
					print
					  qq{<td class="tdc"><font color="green">$corr</font></td>};
				}
...
qq{<font color="red">No Platform available!  Please contact administrator.</font>};
	...
	my $plottype =
	  ...
	  : '';
	my $testcol =
	  ( $testgdb::webUtil::frmData{testcol} )
	  ? $testgdb::webUtil::frmData{testcol} - 1
	  : '';    #subtract 1 because file is 0-based
	my $testbkgd =
	  ( $testgdb::webUtil::frmData{testbkgd} )
	  ? $testgdb::webUtil::frmData{testbkgd} - 1
	  : '';
	my $cntlcol =
	  ( $testgdb::webUtil::frmData{cntlcol} )
	  ? $testgdb::webUtil::frmData{cntlcol} - 1
	  : '';
	my $cntlbkgd =
	  ( $testgdb::webUtil::frmData{cntlbkgd} )
	  ? $testgdb::webUtil::frmData{cntlbkgd} - 1
	  : '';

	if ( !$testcol and $plottype =~ /maplot|xyplot/ )
	{
		print
		  qq{<font color="red">Test data required for M/A or X/Y plots!</font>};
		return;
	}
	...
	for my $bioassays_id ( sort keys %sample )
	{
		...
		my ( %testdata, %cntldata, @maxval, %plotTestdata, %plotCntldata,
			 @stddata )
		  = ();

		open( FILE,
"$testgdb::util::datapath/$sample{$bioassays_id}{accession}/$sample{$bioassays_id}{fname}"
		);
...
#antilog - convert log 10 to base number, log10(num)=val  antilog== 10^val = num
			...
			#czhang
			if($testVal ne '' and looks_like_number($testVal)){
			push @maxval, $testVal;
			}
...
qq{<font color="red">ID refs are inconsistent between platform and soft xml !</font>\n};
		...
		#czhang
		my @trmArray=testgdb::util::transformArray(@maxval);

		my $needtoLog = ( max(@trmArray) > 24 ) ? 1 : 0;
		if ( !$log and $needtoLog )
		{
			print
qq{<font color="red">Data does not seem to be Log values? (maximum value > 24)</font>\n};
...
qq{<tr><th class="thc" colspan="8">$sample{$bioassays_id}{sampid}</th></tr>\n},
		  ...
		  qq{<tr>},                          qq{<th class="thc">DATA</th>},
		  qq{<th class="thc">NO DATA</th>},  qq{<th class="thc">TOTAL</th>},
		  qq{<th class="thc">PLATFORM</th>}, qq{<th class="thc">STDDEV</th>},
		  qq{<th class="thc">MEAN</th>},     qq{<th class="thc">MIN</th>},
		  qq{<th class="thc">MAX</th>}, qq{</tr>}, qq{<tr bgcolor="#EBF0F2">},
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$testgdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=$sample{$bioassays_id}{sampid}','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
			 ...
			 and $testgdb::webUtil::useracclevel > 2 )
		{
			...
			$savExpInfo{expmean} = $mean;
			savExp(
					$testgdb::webUtil::frmData{id},
					$bioassays_id, $dataFilename,
					\%chanSource,  \%savExpInfo
			);
		}
	}    #end of all samples
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$testgdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=Average ($avgsname)','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
		...
		if ( $testgdb::webUtil::useracclevel > 2 )
		{
			...
			savExp( $testgdb::webUtil::frmData{id},
					$testgdb::webUtil::frmData{id},
					$dataFilename, \%chanSource, \%savExpInfo );
		}
	}    #end average
}
...
sub writeDataToFile
{
	...
	my $geneLocRef =
	  testgdb::oracle::dbgetGeneLoc($genomeacc);    #genome gene and start
	...
	for my $id_ref ( sort keys %plotCntldata )
	{
		my $gene = $id_ref;
		print DOUT
		  "$gene\t$id_ref\t$plotTestdata{$id_ref}\t$plotCntldata{$id_ref}\n";
	}
	close DOUT;
		...
		} else
		{
			if ( exists $NSgeneLoc{$id_ref} )
			{
				$gene = $geneLoc{ $NSgeneLoc{$id_ref} }{gene}
				  if $geneLoc{ $NSgeneLoc{$id_ref} }{gene};
			}
		}
		print DOUT
		  "$gene\t$id_ref\t$plotTestdata{$id_ref}\t$plotCntldata{$id_ref}\n";
...
sub viewPlotData
{
	...
	print
qq{<html><head><title>$testgdb::webUtil::frmData{samp} Plot Data</title>\n},
	  qq{<link rel="stylesheet" type="text/css" href="/web/css/main.css">\n},
...
sub savExptoDB
{
	...
	{
		print
qq{<font color="green">* * * Experiment added successfull. * * *</font>};
	} else
	{
		print
		  qq{<font color="red">* * * Error adding experiment!! * * *</font>};
	}
